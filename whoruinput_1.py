...
print('Number of all possible combinations: ',len(groups_of_five_indexes))
...
def sum_of_dot_product(vectors):
    ...
    for i in range(5):
        for j in range(5):
            if i != j:
                #print(i,j)
                #print('Dot product of ',np.dot(vectors[i], vectors[j]))
...
for indexes in groups_of_five_indexes:
    ...
    # Refresh vectors after every iteration
    vectors = []
    #print(indexes)
    #print('5 groups:', np.array(df['Names'])[list(indexes)])
